...
def get_credentials():
    ...
    if len(sys.argv) >= 3:
        ...
        print("[WEB MODE] Credentials received from panel")
        print(f"[UID] {uid}")

        return uid, password

    # 🔥 2. File fallback
    print("[FILE MODE] Loading credentials from file...")
...
"""
                                ...
                                credentials = load_credentials_from_file("MAINUL9X00.txt")
                                if not credentials:
...
async def MaiiiinE():
# 🔐 LOAD CREDENTIALS (WEB + FILE SUPPORT)
print("🔐 Loading credentials...")

# 👉 1. Web panel (priority)
if len(sys.argv) >= 3:
    Uid = sys.argv[1]
    Pw = sys.argv[2]
    print("🌐 Using Web Panel Credentials")
...
# 👉 2. File fallback
    ...
    if not credentials:
        ...
        print("💡 Please create MAINUL9X.txt with your UID and password")
...
print(f"✅ Credentials Ready → UID: {Uid}")
...
# ===============================
# MAINUL - X BOT (WEB CONSOLE VERSION)
# ===============================
...
async def MaiiiinE():
    ...
    # 🔥 ONLY WEB INPUT
    if len(sys.argv) < 3:
        print("❌ No UID/PASSWORD from web panel!")
        return None

    Uid = sys.argv[1]
    Pw = sys.argv[2]

    print(f"🌐 Using Web Panel → UID: {Uid}")
    ...
    # START BOT
    # ===============================
    await StarTinG(auth, token, url, Uid)

# ===============================
# BOT FUNCTION
# ===============================
async def StarTinG(auth, token, UrL, Uid):
    ...
    LoGinDaTa = await GetLoginData(UrL, None, token)
    ...
    print(f"🌐 Server: {OnLineiP}:{OnLineporT}")
